        ...
        ### Testing ###
            ...
            if test_res is not None:
                logger.info("Test - Best epoch, Loss, MSE, RMSE, MAE, MAPE: {}, {:.5f}, {:.5f}, {:.5f}, {:.5f}, {:.2f}%".format(
                    best_iter, test_res["loss"], test_res["mse"], test_res["rmse"],
                    test_res["mae"], test_res["mape"] * 100
                ))
            logger.info(f"Time spent: {time.time() - st:.2f}s")

        if itr - best_iter >= args.patience:
            print("Exp has been early stopped!", file=sys.stderr)
            sys.exit(0)
